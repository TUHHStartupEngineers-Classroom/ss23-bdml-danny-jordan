...
# Data transformation
  ...
  # Filter for dates beginning 2018 and beyond
  filter(year(date) >= 2018) %>%
  
  # Group by symbol
  group_by(symbol) %>%
  
  # Compute a lag of 1 day on the adjusted stock price and adds as new column
...
#tot.withinss (Total Within Sum of Squares): This is a measure of how well the K-Means clusters represent the data. It is the sum of the squared distances from each data point to its cluster center (the mean of the data points in that cluster). A smaller tot.withinss value indicates that the data points are close to their cluster centers, indicating that the model represents the data well.
glance_result <- glance(kmeans_obj)
glance_result
glance_result$tot.withinss
```
...
umap_kmeans_results_tbl %>%
  ...
  theme_tq() +
  labs(title = "K-Means (K = 10) + UMAP Projection")
...
############## JUST FOR MYSELF #########
  ...
  #Getting the data:
# First, I got the price data of 500 stocks in the S&P 500. Each of these stocks had information on daily returns.

  #Restructure Data: 
#Then I reorganized the data a bit. I ended up with a table where each row was a stock and each column was a date. The cells in the table showed the daily changes in returns.

  #K-Means Clustering:
# after preparing the data this way, I unleashed the K-Means algorithm on it. The goal was to put similarly fluctuating stocks into common clusters. So basically, to bring together stocks that behave similarly.

  #Finding Optimal K: 
#For the K-Means algorithm, I had to say how many clusters it should form (that's the "K"). To figure out the best K, I used the "scree plot" method. Here I kept increasing K and seeing 
